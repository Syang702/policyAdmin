<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="floor" label="发奖代数" align="center"></el-table-column>
  ...
  methods: {
    ...
    addSubmit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.listLoading = true;
          addIntroAwards(this.awardsForm).then(resp => {
            this.getList();
            this.listLoading = false;
    ...
    updateSubmit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.listLoading = true;
          introAwardsSubmit(this.awardsForm).then(resp => {
            this.getList();
            this.listLoading = false;
            this.awardsDialog = false;
          });
    ...
    deleteHandle(id) {
      this.$confirm("此操作将永久删除该条信息, 是否继续?", "提示", {
        ...
        .then(() => {
          deleteAwardsSubmit(id).then(response => {
            this.getList();
          });
        })
        .catch(() => {
