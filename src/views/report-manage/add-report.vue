      ...
      >
        ...
        <el-form-item label="报表信息">
          <div class="declare" v-for="(item, index) in declare" :key="index">
            报表<el-input v-model="item.title"></el-input> 条件<el-input
...
<script>
import { addReport, editReport, editSubmit } from "@/api/report-manage";
    ...
    editForm() {
      ...
      editReport(id).then(res => {
        ...
        res.detail.forEach(item => {
          array.push({ title: item.title, conditions: item.conditions });
        });
        this.declare = array;
      });
    },
    ...
    submitAddForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$set(this.dialogForm, "score", JSON.stringify(this.declare));
          addReport(this.dialogForm).then(res => {
            this.$router.go(-1);
          });
        }
      });
